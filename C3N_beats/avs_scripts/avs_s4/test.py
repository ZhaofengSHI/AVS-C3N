...
class audio_extractor(torch.nn.Module):
    def __init__(self, cfg, device):
        ...
        self.audio_backbone_check = torch.load(cfg.TRAIN.PRETRAINED_BEATS_PATH)
        self.cfg = BEATsConfig(self.audio_backbone_check['cfg'])

        self.audio_backbone = BEATs(self.cfg)
        self.audio_backbone.load_state_dict(self.audio_backbone_check['model'])

    def forward(self, audio):

        padding_mask = torch.zeros_like(audio).bool().cuda()

        audio_fea,audio_cls = self.audio_backbone.extract_features(audio, padding_mask=padding_mask)#[0]
...
if __name__ == "__main__":
    ...
    for n_iter, batch_data in enumerate(test_dataloader):
        ...
        if count == 1:
            ...
            outputs.append(output)
            masks.append(mask)
            output_bs4 = torch.cat(outputs,dim=0)
            mask_bs4 = torch.cat(masks, dim=0)

            print(output_bs4.shape,mask_bs4.shape)

            # eval
            miou = mask_iou(output_bs4.squeeze(1), mask_bs4)
            avg_meter_miou.add({'miou': miou})
            F_score = Eval_Fmeasure(output_bs4.squeeze(1), mask_bs4, log_dir)
